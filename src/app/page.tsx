...
const moodCards = [
  {
    title: '배달각 온 밤',
    label: '피곤함 · 보상심리',
    body: '카피가 10초만 같이 쉬어줘요. 참는 앱이 아니라 덜 무너지는 앱.',
    icon: 'ramen_dining',
    bg: 'bg-[#fff1e6]',
  },
  {
...
const quickActions = [
  { href: '/survey', icon: 'favorite', title: '카피랑 10초 쉬기', desc: '지금 마음을 고르고 오늘의 카피로그를 만들어요' },
  { href: '/memo', icon: 'edit_note', title: '소비/투자 로그 남기기', desc: '왜 눌렀는지, 어떤 마음이었는지 가볍게 기록해요' },
  ...
  return (
    <div className="animate-slideUp space-y-10">
      <section className="relative overflow-hidden rounded-[44px] border border-surface-border bg-[radial-gradient(circle_at_76%_20%,#ffd8c6_0_18%,transparent_38%),radial-gradient(circle_at_20%_85%,#e3f1e4_0_20%,transparent_42%),linear-gradient(135deg,#fffaf2_0%,#fff0df_100%)] p-7 shadow-elevated md:p-11">
        <div className="absolute right-8 top-8 rounded-full bg-white/60 px-4 py-2 text-xs font-black text-primary shadow-raised backdrop-blur">Dear ANT × Capi coach</div>
        <div className="grid items-center gap-10 md:grid-cols-[1.05fr_0.95fr]">
          <div>
            ...
            <p className="mt-6 max-w-[580px] text-xl font-black leading-8 text-on-surface-secondary md:text-2xl">
              괜찮아. 일단 카피랑 10초만 쉬자.
            </p>
            <p className="mt-4 max-w-[610px] text-base font-semibold leading-8 text-on-surface-tertiary">
              Dear ANT는 소비와 투자 충동을 혼내지 않고, 카피바라 코치가 감정 스티커와 Future Me Card로 바꿔주는 귀여운 습관 앱입니다.
            </p>
          ...
          <div className="relative mx-auto w-full max-w-[380px]">
            ...
            <div className="rounded-[42px] border border-surface-border bg-white/82 p-6 shadow-[0_24px_70px_rgba(62,42,25,0.16)] backdrop-blur">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  <p className="text-sm font-black text-primary">오늘의 카피 상태</p>
              ...
              <div className="mt-5 rounded-[28px] bg-primary-container p-5">
                <p className="text-xs font-black text-primary">Capi says</p>
      ...
      <section className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="card card-raised">
          <p className="section-label">Capi loop</p>
          <h2 className="text-2xl font-black tracking-[-0.04em]">누르기 전에 귀엽게 한 번 멈추는 루프.</h2>
          <div className="mt-6 space-y-4">
            {['충동이 올라옴', '카피가 10초 쉬자고 함', '감정 스티커 선택', '작은 미래 행동', 'Future Me Card 저장'].map((step, index) => (
